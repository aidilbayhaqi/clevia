...
export const crmApi={
  async login(email,password){
    ...
    return {...d,user:normalizeUser(d.user)};
  },
  ...
  async appointments(){
    ...
    const [rows,clients,leads,services,staff]=await Promise.all([
      request('/appointments',{auth:true}),
      request('/crm/clients',{auth:true}),
      request('/crm/leads',{auth:true}),
      request('/public/services'),
      request('/public/staff'),
    ]);
    const cm=new Map(clients.map(x=>[x.id,x.full_name]));
    const lm=new Map(leads.map(x=>[x.id,x.full_name]));
    const sm=new Map(services.map(x=>[x.id,x.name]));
    const dm=new Map(staff.map(x=>[x.id,x.full_name]));
    return rows.map(x=>({
      ...x,
      client_name:cm.get(x.client_id)||lm.get(x.lead_id)||'Unlinked client',
      service_name:sm.get(x.service_id)||x.service_id,
      staff_name:dm.get(x.staff_id)||x.staff_id,
      scheduled_at:x.starts_at,
    }));
  },
  ...
  async conversations(){
    ...
    return rows.map(x=>({
      ...x,
      visitor_name:cm.get(x.client_id)||lm.get(x.lead_id)||`Visitor ${String(x.id).slice(0,6)}`,
      last_message:x.handoff_summary||'Open conversation',
      updated_at:x.updated_at||x.created_at,
    }));
  },

  transcript:(id)=>DEMO_MODE?wait(messages[id]||[]):request(`/conversations/${id}/messages`,{auth:true}),
  ...
  resolve:(id)=>DEMO_MODE?wait({id,status:'resolved'}):request(`/conversations/${id}/resolve`,{method:'POST',auth:true}),
  ...
  async reply(id,message){
    ...
    return request(`/conversations/${id}/messages`,{method:'POST',auth:true,body:{message}});
  },

  feedback:(conversationId,messageId,rating,note=null)=>DEMO_MODE
    ?wait({id:`demo-feedback-${Date.now()}`,message_id:messageId,rating,note})
    :request(`/conversations/${conversationId}/messages/${messageId}/feedback`,{
      method:'POST',auth:true,body:{rating,note},
    }),
  ...
  approveKnowledge:(id)=>DEMO_MODE?wait({id,status:'approved'}):request(`/knowledge/${id}/approve`,{method:'POST',auth:true}),
  publishKnowledge:(id)=>DEMO_MODE?wait({id,status:'approved'}):request(`/knowledge/${id}/approve`,{method:'POST',auth:true}),
};
