...
async def _validate_booking_rules(
    ...
    *,
    ...
    # Clinic MVP currently uses Asia/Jakarta. The public/agent availability tool
    # returns timezone-aware datetimes, so this validates against the staff window.
...
async def create_appointment(
    ...
    *,
    ...
    # Serialize booking writes per practitioner inside the PostgreSQL transaction.
    # This prevents two concurrent requests from both passing the conflict check.
    ...
    # Availability is intentionally cached only briefly, but write operations
    # invalidate all availability keys for this clinic immediately.
    await cache_delete_pattern(
        cache_key("availability", clinic_id, "*")
    )
